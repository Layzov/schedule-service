...
Короткая инструкция для первого запуска проекта на локальной машине (ориентирована на Windows / PowerShell, но под Unix команды работают так же).
...
Эта цель выполнит в порядке: загрузку/обновление модулей (`go mod download` / `go mod tidy`), поднятие `docker-compose` (Postgres и другие сервисы), применение SQL-миграций и сборку + запуск локального бинаря.
...
- `make start` — поднять сервисы и применить миграции (docker)
- `make start-local` — то же, затем собрать и запустить локальный бинарь
...
## Принятые решения и замечания по реализации
